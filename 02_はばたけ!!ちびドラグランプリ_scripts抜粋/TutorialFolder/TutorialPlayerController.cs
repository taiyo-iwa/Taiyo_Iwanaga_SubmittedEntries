...
public class TutorialPlayerController : MonoBehaviour
{
    ...
    [SerializeField] private TutorialPlayerItem _item = default;
    ...
    [SerializeField] private TutorialPlayerMovement _playerMovement = default;
    ...
    public void Controller()
    {
        ...
        if (_groundCheck.IsGrounded && !_pendingDrift && !_isDrifting)
        {
            if (!_tutorialManager._eventStop)
            {
                if (_RBbtnDown)
                {
        ...
        if (!_tutorialManager._eventStop)
        {
            ...
            if (_tutorialManager._dashEventFlag)
            {
                if (_isTrigger && _canDashing)
                {
        ...
        if(!_tutorialManager._shieldExplanation && !_tutorialManager._isLTbtnEvent && _tutorialManager._isLBbtnEvent)
        {
            if (_LBbtn)
            {
                _item.UseItem(TutorialPlayerItem.ItemUseType.Attack);
            }
        }
        if(!_tutorialManager._frameExplanation && !_tutorialManager._isLBbtnEvent && _tutorialManager._isLTbtnEvent)
        {
            if (_leftTriggrt > 0)
            {
                _item.UseItem(TutorialPlayerItem.ItemUseType.Defense);
            }
        } 
        #endregion
        //ゲージが溜まり飛べる時
        if (_flightGauge.CheckCanUseGauge())
        {
            _tutorialManager._canSoar = true;
        }
    }
