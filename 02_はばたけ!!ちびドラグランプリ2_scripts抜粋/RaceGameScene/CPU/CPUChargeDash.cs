...
public class CPUChargeDash : MonoBehaviour
{
    //最低限のチャージ時間
    private const float MINIMUM_CHARGE_TIME = 0.3f;
    //最大チャージ時間
    ...
    private bool _isCPUBrake = false;
    private bool _wasCharging = false;
    private float _chargeTime = 0;

    public void CPUChargeInput(bool southButtonInput)
    {
        _isCPUBrake = southButtonInput;
    }

    public void UpdateCPUChargDash()
    {
        CPUChargeStart();
    }

    private void CPUChargeStart()
    {
        if (_isCPUBrake)
        {
    ...
    private void TryMiniTurbo()
    {
        ...
        if (!_cpuStatus.CanReleaseCharge)
        {
            return;
        }
        ...
        //チャージダッシュしたことを通知
        _cpuStatus.NotifyStartChargeDash();
    }
}
